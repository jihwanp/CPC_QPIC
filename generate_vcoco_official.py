...
from vcoco_official_eval import VCOCOeval
...
def get_args_parser():
    ...
    parser.add_argument('--path_id', default=0, type=int,help= 'order of used paths')
    # * HOI
...
def main(args):
    ...
    path_name=args.save_path+'_'+str(args.path_id)+'.pickle'
    with open(path_name, 'wb') as f:
        pickle.dump(detections, f, protocol=2)
    
    vcocoeval = VCOCOeval('data/v-coco/vcoco/vcoco_test.json', 'data/v-coco/data/instances_vcoco_all_2014.json', 'data/v-coco/splits/vcoco_test.ids')
    vcocoeval._do_eval(path_name, ovr_thresh=0.5)

@torch.no_grad()
def generate(model, post_processor, data_loader, device, verb_classes, missing_category_id,path_id):
    model.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        results = post_processor(outputs, orig_target_sizes,path_id)
