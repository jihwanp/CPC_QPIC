...
class HungarianMatcherHOI(nn.Module):
    ...
    def forward(self, outputs, targets,log=False):
        if log:
            log_dict = {'obj_class': [], 'verb_class': [], 'bbox': [], 'giou':[]}

        bs, num_path,num_queries = outputs['pred_obj_logits'].shape[:3]
        return_list=[]
        for batch_idx in range(bs):
            out_obj_prob = outputs['pred_obj_logits'][batch_idx].flatten(0, 1).softmax(-1)
            out_verb_prob = outputs['pred_verb_logits'][batch_idx].sigmoid()
            out_verb_prob=out_verb_prob.flatten(0,1)
            out_sub_bbox = outputs['pred_sub_boxes'][batch_idx].flatten(0, 1)
            out_obj_bbox = outputs['pred_obj_boxes'][batch_idx].flatten(0, 1)

            tgt_obj_labels = targets[batch_idx]['obj_labels'].repeat(num_path)
            tgt_verb_labels = targets[batch_idx]['verb_labels'].repeat(num_path,1)
            tgt_verb_labels_permute = tgt_verb_labels.permute(1, 0)
            tgt_sub_boxes = targets[batch_idx]['sub_boxes'].repeat(num_path,1)
            tgt_obj_boxes = targets[batch_idx]['obj_boxes'].repeat(num_path,1)
            ...
            sizes = [len(targets[batch_idx]['obj_labels'])]*num_path
            indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
            return_list.append([(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices])
